...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        /*
         * LAB8: initialize the per-process file table pointer. New
         * processes must not assume any valid files_struct until
         * do_fork installs one. Leaving this pointer uninitialized
         * would propagate stale heap data into the file system layer
         * and cause unpredictable behaviour.  See do_fork for how it
         * gets assigned.
         */
        proc->filesp = NULL;
    }
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    /*
     * LAB8: copy the file descriptor table from the current process
     * into the new child.  If CLONE_FS is set in clone_flags then
     * copy_files will share the parent's table by simply incrementing
     * the reference count.  Otherwise it will duplicate the table.
     * On failure the resources allocated so far must be released.
     */
    if (copy_files(clone_flags, proc) != 0) {
        // free kernel stack and the process structure on failure
        put_kstack(proc);
        kfree(proc);
        return -E_NO_MEM;
    }

    // Now that mm and file table are set up, initialize the trap frame and
    // thread context for the new process.
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        // establish parent/child relationships in the process tree
        set_links(proc);
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    /*
     * For exercise 0 of lab8 we do not yet support loading programs
     * directly from the file system.  Returning an unimplemented error
     * here will cause sys_exec to fail gracefully.  A complete
     * implementation should follow the outline above (see lab5's
     * exec implementation) to parse the ELF header from the file
     * descriptor, allocate and map memory for program segments, copy
     * arguments onto the user stack, and initialize the trap frame.
     */
    return -E_UNIMP;
}
